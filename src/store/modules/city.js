...
const getters = {
  ...
  getCityId: (state) => (id) => {
    return state.cities.find((cities) => cities.id === id)
  }
}
...
const actions = {
  ...
  async updateCity ({ state, commit, dispatch }, city) {
    try {
      const cityEdit = await CityService.update(city.id, city)
  ...
  async getCities ({ commit }) {
    try {
      ...
      // citiesData.data.map(function (data) {
      //   data.activeText = data.active == 1 ? "Activo" : "Inactivo";
      // });
      commit('setCities', citiesData.data)
    } catch (error) {
      console.log(error)
    }
  }
